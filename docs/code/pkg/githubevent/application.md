...
- [type StatusWriter](<#StatusWriter>)
  ...
  - [func \(w \*StatusWriter\) Write\(ctx context.Context, ev \*eventsv1alpha1.GitHubEvent, result domain.GitHubEventResult, runErr error\) error](<#StatusWriter.Write>)
...
```go
func (w *StatusWriter) Write(ctx context.Context, ev *eventsv1alpha1.GitHubEvent, result domain.GitHubEventResult, runErr error) error
```

Write persists the provider result and any error to the GitHubEvent CR. A non\-nil runErr marks the event as not accepted and not triggered.
