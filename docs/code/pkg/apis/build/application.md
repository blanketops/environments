...
The caller's Build object is used only as a lookup key for its conditions — its local status is NOT refreshed from the re\-fetched latest version. The one exception is Status.Contract: if the caller has populated it \(buildrun\-observer does, to carry ExecutionRef/BuildHash/Success/Triggered for build\-observer's applyRetry to read back\), it is persisted verbatim alongside the conditions. Callers that leave Contract empty \(the primary BuildService.Reconcile path today\) see no change in behavior.
