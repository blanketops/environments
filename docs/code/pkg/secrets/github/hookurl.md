...
<a name="HookURLSecretKey"></a>HookURLSecretKey is the key under which the webhook URL is stored in the reconciled Secret's data.
...
Reconcile creates or updates the hookurl Secret for resolved's repository from its contract\-declared hookUrl.
