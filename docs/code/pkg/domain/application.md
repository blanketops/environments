...
Write merges the provided conditions into the Domain CR's status and persists. Scalar fields \(Phase, DomainReady, TLSReady, CertificateRef\) must be set on the CR by the caller before Write is invoked. This method only merges and writes.
