...
func newResolvedPackageWithStateRepo(name, namespace, secretName string) *packageResolution.ResolvedPackage {
	return &packageResolution.ResolvedPackage{
		...
		Spec: &packageResolution.ResolvedPackageSpec{
			StateRepository: &packageResolution.ResolvedStateRepository{CloneSecret: secretName},
		},
	}
}

func TestPackageStateRepositorySecretReconciler_Reconcile_NilGuards(t *testing.T) {
	c := fake.NewClientBuilder().WithScheme(testutil.NewScheme(t, environmentv1alpha1.AddToScheme)).Build()
	r := NewPackageStateRepositorySecretReconciler(c, logr.Discard(), "vault-store", "ClusterSecretStore")
	...
	empty := newResolvedPackageWithStateRepo("pkg1", "default", "")
	if err := r.Reconcile(context.Background(), empty); err != nil {
		t.Fatalf("empty CloneSecret should no-op, got: %v", err)
	}
}

func TestPackageStateRepositorySecretReconciler_Reconcile_Creates(t *testing.T) {
	c := fake.NewClientBuilder().WithScheme(testutil.NewScheme(t, environmentv1alpha1.AddToScheme)).Build()
	r := NewPackageStateRepositorySecretReconciler(c, logr.Discard(), "vault-store", "ClusterSecretStore")
	pkg := newResolvedPackageWithStateRepo("pkg1", "default", "pkg1-state-repo")

	if err := r.Reconcile(context.Background(), pkg); err != nil {
		t.Fatalf("Reconcile: %v", err)
	}

	es := testutil.GetExternalSecret(t, c, "pkg1-state-repo", "default")
...
func TestPackageStateRepositorySecretReconciler_Reconcile_UpdatesOnDrift(t *testing.T) {
	c := fake.NewClientBuilder().WithScheme(testutil.NewScheme(t, environmentv1alpha1.AddToScheme)).Build()
	r := NewPackageStateRepositorySecretReconciler(c, logr.Discard(), "vault-store", "ClusterSecretStore")
	pkg := newResolvedPackageWithStateRepo("pkg1", "default", "pkg1-state-repo")
	...
	r2 := NewPackageStateRepositorySecretReconciler(c, logr.Discard(), "new-store", "ClusterSecretStore")
...
func TestPackageStateRepositorySecretReconciler_Reconcile_NoopWhenUpToDate(t *testing.T) {
	c := fake.NewClientBuilder().WithScheme(testutil.NewScheme(t, environmentv1alpha1.AddToScheme)).Build()
	r := NewPackageStateRepositorySecretReconciler(c, logr.Discard(), "vault-store", "ClusterSecretStore")
	pkg := newResolvedPackageWithStateRepo("pkg1", "default", "pkg1-state-repo")
	...
	before := testutil.GetExternalSecret(t, c, "pkg1-state-repo", "default")

	if err := r.Reconcile(context.Background(), pkg); err != nil {
		t.Fatalf("second Reconcile: %v", err)
	}
	after := testutil.GetExternalSecret(t, c, "pkg1-state-repo", "default")
...
func TestPackageStateRepositorySecretReconciler_Delete_NilGuards(t *testing.T) {
	c := fake.NewClientBuilder().WithScheme(testutil.NewScheme(t, environmentv1alpha1.AddToScheme)).Build()
	r := NewPackageStateRepositorySecretReconciler(c, logr.Discard(), "vault-store", "ClusterSecretStore")
...
func TestPackageStateRepositorySecretReconciler_Delete_RemovesExternalSecretAndSecret(t *testing.T) {
	c := fake.NewClientBuilder().WithScheme(testutil.NewScheme(t, environmentv1alpha1.AddToScheme)).Build()
	r := NewPackageStateRepositorySecretReconciler(c, logr.Discard(), "vault-store", "ClusterSecretStore")
	pkg := newResolvedPackageWithStateRepo("pkg1", "default", "pkg1-state-repo")
	...
	secret := &corev1.Secret{ObjectMeta: metav1.ObjectMeta{Name: "pkg1-state-repo", Namespace: "default"}}
	...
	if testutil.ExternalSecretExists(t, c, "pkg1-state-repo", "default") {
		t.Fatal("expected ExternalSecret to be deleted")
	}
	if testutil.SecretExists(t, c, "pkg1-state-repo", "default") {
		t.Fatal("expected Secret to be deleted")
	}
}

func TestPackageStateRepositorySecretReconciler_Delete_IdempotentWhenNotFound(t *testing.T) {
	c := fake.NewClientBuilder().WithScheme(testutil.NewScheme(t, environmentv1alpha1.AddToScheme)).Build()
	r := NewPackageStateRepositorySecretReconciler(c, logr.Discard(), "vault-store", "ClusterSecretStore")
	pkg := newResolvedPackageWithStateRepo("pkg1", "default", "pkg1-state-repo")
