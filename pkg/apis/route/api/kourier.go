...
/*
This file owns KourierProvider — the Knative implementation of the routes
Provider interface.

KourierProvider materializes a Route as a Knative DomainMapping
...
See also:
  ...
  - pkg/route/application/backend_selector.go — wires KourierProvider for RuntimeKnativeService
*/
package api
...
func NewKourierProvider(c client.Client, log logr.Logger) *KourierProvider {
	return &KourierProvider{client: c, log: log}
}
...
func (p *KourierProvider) ensureDisabled(ctx context.Context, route domain.Route) (domain.RouteResult, error) {
	dm := &knservingv1beta1.DomainMapping{}
