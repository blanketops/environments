...
func TestToServiceUnitContract_NilSpec(t *testing.T) {
	var s *resolve.ResolvedServiceUnitSpec
	if got := ToServiceUnitContract(s); got != nil {
		t.Fatalf("expected nil, got %+v", got)
	}
}

func TestToServiceUnitContract_Static(t *testing.T) {
	s := &resolve.ResolvedServiceUnitSpec{
...
func TestToServiceUnitContract_Build(t *testing.T) {
	s := &resolve.ResolvedServiceUnitSpec{
		...
		BuildRef: &resolve.ResolvedBuildRef{Name: "b1", Namespace: "default"},
	}
	out := ToServiceUnitContract(s)
...
func TestToServiceUnitContract_UnknownType(t *testing.T) {
	s := &resolve.ResolvedServiceUnitSpec{Type: commoncontractv1.ServiceUnitType_SERVICE_UNIT_TYPE_UNSPECIFIED}
	out := ToServiceUnitContract(s)
