...
This file owns the Adapter — a thin struct wrapper around resolve.ResolveServiceUnit.
The Adapter exists to satisfy interface-based injection points in the
application layer where a concrete resolution dependency must be passed as a
value rather than called as a free function.
...
// Stateless — all resolution logic lives in ResolveServiceUnit.
//
...
// Resolve delegates to ResolveServiceUnit, returning the resolved ServiceUnit contract
// or an error if the CR spec fails validation.
func (a *Adapter) Resolve(
	ctx context.Context,
	serviceunit *environmentv1alpha1.ServiceUnit,
) (*resolve.ResolvedServiceUnit, error) {
	return resolve.ResolveServiceUnit(serviceunit)
}
