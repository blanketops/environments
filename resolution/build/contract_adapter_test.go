...
func TestToBuildContract_NilSpec(t *testing.T) {
	var s *ResolvedBuildSpec
	if got := s.ToBuildContract(); got != nil {
		t.Fatalf("expected nil for nil spec, got %+v", got)
	}
}

func TestToBuildContract_Minimal(t *testing.T) {
	s := &ResolvedBuildSpec{Image: "foo:latest"}
	out := s.ToBuildContract()
...
func TestToBuildContract_FullyPopulated(t *testing.T) {
	s := &ResolvedBuildSpec{
		...
		Strategy: ResolvedStrategy{Name: "buildpacks", StrategyKind: "ClusterBuildStrategy"},
		ServiceAccount: &ResolvedServiceAccount{
			Name:   "sa",
			Secret: "sa-secret",
		},
		Policy: &ResolvedBuildPolicy{
			Retry: &ResolvedRetryPolicy{OnFailure: true, MaxAttempts: 3},
		},
	}
...
func TestToBuildContract_PolicyWithoutRetry(t *testing.T) {
	s := &ResolvedBuildSpec{Image: "foo", Policy: &ResolvedBuildPolicy{}}
	out := s.ToBuildContract()
	if out.Policy != nil {
