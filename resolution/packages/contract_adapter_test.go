...
func TestToPackageContract_NilSpec(t *testing.T) {
	var s *ResolvedPackageSpec
	if got := s.ToPackageContract(); got != nil {
		t.Fatalf("expected nil, got %+v", got)
	}
}

func TestToPackageContract_Minimal(t *testing.T) {
	s := &ResolvedPackageSpec{
		Name:              "pkg1",
		Version:           "1.0.0",
		PackageRepository: ResolvedPackageRepository{URL: "oci://x"},
	}
	out := s.ToPackageContract()
...
func TestToPackageContract_FullyPopulated(t *testing.T) {
	s := &ResolvedPackageSpec{
		...
		DiffEnabled:       true,
		PackageRepository: ResolvedPackageRepository{URL: "oci://x", CredentialsSecret: "sec"},
		StateRepository: &ResolvedStateRepository{
			URL: "git@x", CloneSecret: "clone-sec", Strategy: "kapp", Path: "/state",
		},
		Maintainers: []ResolvedMaintainer{{Name: "neo", Email: "neo@x.com"}},
	}
	out := s.ToPackageContract()
