...
ResolveGitHubPayload function. The Adapter exists to satisfy interface-based injection
...
// Adapter wraps the GitHubPayload resolution function for interface-based injection.
// Stateless — all resolution logic lives in ResolveGitHubPayload.
//
...
// Resolve delegates to ResolveGitHubPayload, returning the resolved GitHubPayload contract
// or an error if the CR spec fails validation.
func (a *Adapter) Resolve(
	ctx context.Context,
	githubpayload *eventsv1alpha1.GitHubPayload,
) (*ResolvedGitHubPayload, error) {
	return ResolveGitHubPayload(githubpayload)
}
