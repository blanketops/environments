...
runtime spec (ResolvedGitHubPayloadSpec) to the protobuf contract type
(eventscontractv1alpha1.GitHubPayloadSpec).
...
UNSPECIFIED rather than returning an error, for the same reason as the
GitHubEvent adapter: GitHub may deliver event types the platform has not yet
modelled.

ReceivedAt is intentionally left unset, mirroring how the GitHubEvent adapter
also omits OccurredAt — the receiver here is the Spec alone, which carries no
timestamp; the CR's own metadata.creationTimestamp already covers this.
*/
...
// ToGitHubPayloadContract projects the resolved runtime GitHubPayload spec
// into a protobuf eventscontractv1alpha1.GitHubPayloadSpec for infrastructure
...
// from the Sensor's trigger parameters — no transformation applied.
//
...
func (s *ResolvedGitHubPayloadSpec) ToGitHubPayloadContract() *eventscontractv1alpha1.GitHubPayloadSpec {
	...
	return &eventscontractv1alpha1.GitHubPayloadSpec{
		...
		Actor:          s.Actor,
		EventId:        s.EventID,
		GithubEventRef: s.GitHubEventRef,
	}
}
