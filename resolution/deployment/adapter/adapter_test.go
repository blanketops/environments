...
func TestAdapter_Resolve(t *testing.T) {
	...
	if resolved.Spec.Strategy != resolve.StrategyRolling {
...
func TestAdapter_Resolve_PropagatesError(t *testing.T) {
	a := NewAdapter()
	if _, err := a.Resolve(context.Background(), &environmentv1alpha1.Deployment{}); err == nil {
		t.Fatal("expected error to propagate from resolve.ResolveDeployment")
	}
}
