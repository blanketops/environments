...
func TestToRouteContract_NilSpec(t *testing.T) {
	out, err := ToRouteContract(&ResolvedRoute{})
	if err != nil || out != nil {
		t.Fatalf("expected (nil, nil), got (%+v, %v)", out, err)
	}
}
...
func TestToRouteContract_Populated(t *testing.T) {
	...
	r := &ResolvedRoute{
		Spec: &ResolvedRouteSpec{
			...
			TLSEnabled:     true,
			Runtime:        &rt,
			ServiceUnitRef: &ResolvedServiceUnitRef{Name: "su1"},
		},
	}
...
func TestMapRuntime(t *testing.T) {
	knative := RuntimeKnativeService
	k8s := RuntimeKubernetesContainer
	gw := RuntimeGatewayAPI
...
func TestMapServiceUnitRef(t *testing.T) {
	...
	got := mapServiceUnitRef(&ResolvedServiceUnitRef{Name: "su1"})
