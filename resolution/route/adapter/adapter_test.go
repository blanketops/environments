...
func TestAdapter_Resolve(t *testing.T) {
	...
	r := &networksv1alpha1.Route{
		Spec: networksv1alpha1.RouteSpec{
			Contract: runtime.RawExtension{Raw: []byte(`{"host":"app.dev.blanketops.online","runtime":"kubernetes-container","serviceUnitRef":{"name":"su1"}}`)},
		},
	}
...
func TestAdapter_Resolve_PropagatesError(t *testing.T) {
	a := NewAdapter()
	if _, err := a.Resolve(context.Background(), &networksv1alpha1.Route{}); err == nil {
		t.Fatal("expected error to propagate from resolve.ResolveRoute")
	}
}
