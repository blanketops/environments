...
func TestToDomainContract_NilSpec(t *testing.T) {
	out, err := ToDomainContract(&resolve.ResolvedDomain{})
	if err != nil || out != nil {
		t.Fatalf("expected (nil, nil), got (%+v, %v)", out, err)
	}
}
...
func TestToDomainContract_Populated(t *testing.T) {
	ts := resolve.TLSStrategyCustom
	d := &resolve.ResolvedDomain{
		Spec: &resolve.ResolvedDomainSpec{
			Host:        "x",
			RouteRef:    resolve.ResolvedDomainRouteRef{Name: "route1"},
			TLSStrategy: &ts,
			MTLS:        resolve.ResolvedDomainMTLS{Enforced: true},
			RenewBefore: "720h",
		},
	}
...
func TestMapTLSStrategy(t *testing.T) {
	platform := resolve.TLSStrategyPlatform
	custom := resolve.TLSStrategyCustom
	unknown := resolve.TLSStrategy("bogus")

	cases := []struct {
		name string
		ts   *resolve.TLSStrategy
...
func TestMapRouteRef(t *testing.T) {
	got := mapRouteRef(resolve.ResolvedDomainRouteRef{Name: "route1"})
	if got.Name != "route1" {
...
func TestMapMTLS(t *testing.T) {
	got := mapMTLS(resolve.ResolvedDomainMTLS{Enforced: true})
	if !got.Enforced {
